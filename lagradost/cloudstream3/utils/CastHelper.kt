...
class CastHelper private constructor() {
    fun getMediaInfo(
        ...
        val extractorLink: ExtractorLink = holder.getCurrentLinks().get(i)
        val mediaMetadata = MediaMetadata(1)
        if (holder.isMovie()) {
            sb = extractorLink.name + ' ' + Qualities.Companion.Companion.getStringByInt(
                Integer.valueOf(extractorLink.quality)
            )
        } else {
            val sb2 = StringBuilder()
            var name: String = epData.getName()
            if (name == null) {
                name = "Episode " + epData.getEpisode()
            }
            sb2.append(name)
            sb2.append(" - ")
            sb2.append(extractorLink.name)
            sb2.append(' ')
            sb2.append(Qualities.Companion.Companion.getStringByInt(Integer.valueOf(extractorLink.quality)))
            sb = sb2.toString()
        }
        mediaMetadata.putString(MediaMetadata.KEY_SUBTITLE, sb)
        val title: String = holder.getTitle()
        if (title != null) {
            mediaMetadata.putString(MediaMetadata.KEY_TITLE, title)
        }
        var poster: String = epData.getPoster()
        if (poster == null) {
            poster = holder.getPoster()
        }
        if (poster != null) {
            mediaMetadata.addImage(WebImage(Uri.parse(poster)))
        }
        var i2 = 0
        val list: List<SubtitleData> = subtitles
        val arrayList = ArrayList(CollectionsKt.collectionSizeOrDefault(list, 10))
        for (subtitleData in list) {
            arrayList.add(
                MediaTrack.Builder(i2, 1).setName(subtitleData.getName()).setSubtype(1)
                    .setContentId(subtitleData.getUrl()).build()
            )
            i2++
        }
        val mediaTracks: MediaInfo.Builder = MediaInfo.Builder(extractorLink.url).setStreamType(1)
            .setContentType(if (extractorLink.isM3u8) MimeTypes.APPLICATION_M3U8 else MimeTypes.VIDEO_MP4)
            .setMetadata(mediaMetadata).setMediaTracks(arrayList)
        if (jSONObject != null) {
            mediaTracks.setCustomData(jSONObject)
        }
        val build: MediaInfo = mediaTracks.build()
    ...
    fun awaitLinks(
        ...
        Coroutines.Companion.INSTANCE.main(`CastHelper$awaitLinks$1`(callback, pendingResult, null))
    }

    /* JADX WARN: Removed duplicated region for block: B:20:0x007a A[Catch: Exception -> 0x00c1, TryCatch #1 {Exception -> 0x00c1, blocks: (B:5:0x0022, B:8:0x0029, B:11:0x0030, B:13:0x0050, B:16:0x0057, B:18:0x005e, B:20:0x007a, B:22:0x0085, B:24:0x008c, B:26:0x009a), top: B:39:0x0022 }] */ /* JADX WARN: Removed duplicated region for block: B:25:0x0099  */ /*
    ...
    */
    fun startCast(
        ...
        r21: String?,
        r22: Int,
        r23: List<ResultEpisode?>?,
        r24: List<ExtractorLink?>?,
        r25: List<SubtitleData?>?,
        r26: Int?,
        r27: Long?
    ): Boolean {
        /*
