...
class `SuperStream$getMainPage$pages$2$postList$1$1`(postJSON: PostJSON) : Lambda<Any?>(1),
    Function1<MovieSearchResponse?, Unit> {
    /* synthetic */ val `$post`: PostJSON
    ...
    fun invoke2(newMovieSearchResponse: MovieSearchResponse) {
        ...
        var poster: String = `$post`.getPoster()
        if (poster == null) {
            poster = `$post`.getPoster2()
        }
        newMovieSearchResponse.posterUrl = poster
        var quality_tag: String = `$post`.getQuality_tag()
        if (quality_tag == null) {
            quality_tag = ""
        }
        newMovieSearchResponse.quality = MainAPIKt.getQualityFromString(quality_tag)
    }
}