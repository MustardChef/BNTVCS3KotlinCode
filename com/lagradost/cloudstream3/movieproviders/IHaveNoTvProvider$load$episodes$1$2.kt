...
internal class `IHaveNoTvProvider$load$episodes$1$2`(/* synthetic */val `$ep`: Element,
                                                                    num: Integer,
                                                                    num2: Integer,
                                                                    str: String
    ...
    fun invoke2(newEpisode: Episode) {
        ...
        val selectFirst: Element = `$ep`.selectFirst("a[title]")
        Intrinsics.checkNotNull(selectFirst)
        newEpisode.name = selectFirst.attr("title")
        newEpisode.setSeason(`$season`)
        newEpisode.setEpisode(`$epNum`)
        newEpisode.posterUrl = `$thumb`
        val selectFirst2: Element = `$ep`.selectFirst(".episodeSynopsis")
        newEpisode.description = selectFirst2?.text()
    }
}