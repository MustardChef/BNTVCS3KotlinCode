...
class `VideoDownloadManager$downloadThing$downloadEventListener$1`(
    num: Integer,
    ...
    object WhenMappings {
        ...
        init {
            ...
            com.lagradost.cloudstream3.utils.iArr.get(DownloadActionType.Pause.ordinal()) = 1
            com.lagradost.cloudstream3.utils.iArr.get(DownloadActionType.Stop.ordinal()) = 2
    ...
    fun invoke2(event: Tuples<Integer?, out DownloadActionType>) {
        ...
        if (num != null && intValue == num.intValue()) {
            ...
            if (i == 1) {
                ...
                VideoDownloadManager.Companion.`downloadThing$updateNotification`(
                    `$isDone`,
                    `$isStopped`,
                    `$isFailed`,
                    `$isPaused`,
                ...
                AcraApplication.Companion.removeKey(
                    "download_resume",
                    String.valueOf(event.getFirst().intValue())
                )
                VideoDownloadManager.Companion.INSTANCE.saveQueue()
            }
        }
    }
}