...
class VisualDownloadHeaderCached constructor(
    ...
    operator fun component5(): DownloadHeaderCached {
        return data
    }

    operator fun component6(): DownloadEpisodeCached? {
        return child
    }
    ...
    fun copy(
        ...
        return VisualDownloadHeaderCached(i, i2, j, j2, data, downloadEpisodeCached)
    }
    ...
    override fun equals(obj: Object): Boolean {
        ...
        if (obj is VisualDownloadHeaderCached) {
            val visualDownloadHeaderCached: VisualDownloadHeaderCached = obj
            return (currentOngoingDownloads == visualDownloadHeaderCached.currentOngoingDownloads) && (totalDownloads == visualDownloadHeaderCached.totalDownloads) && (totalBytes == visualDownloadHeaderCached.totalBytes) && (currentBytes == visualDownloadHeaderCached.currentBytes) && Intrinsics.areEqual(
                data, visualDownloadHeaderCached.data
            ) && Intrinsics.areEqual(
    ...
    override fun hashCode(): Int {
        val hashCode: Int =
            ((((((((currentOngoingDownloads * 31) + totalDownloads) * 31) + `C$r8$backportedMethods$utility$Long$1$hashCode`.hashCode(
                totalBytes
            )) * 31) + `C$r8$backportedMethods$utility$Long$1$hashCode`.hashCode(currentBytes)) * 31) + data.hashCode()) * 31
        val downloadEpisodeCached: DownloadEpisodeCached? = child
        return hashCode + (if (downloadEpisodeCached == null) 0 else downloadEpisodeCached.hashCode())
    }

    override fun toString(): String {
        return "VisualDownloadHeaderCached(currentOngoingDownloads=" + currentOngoingDownloads + ", totalDownloads=" + totalDownloads + ", totalBytes=" + totalBytes + ", currentBytes=" + currentBytes + ", data=" + data + ", child=" + child + ')'
    }

    init {
        Intrinsics.checkNotNullParameter(data, "data")
        currentOngoingDownloads = i
        totalDownloads = i2
        totalBytes = j
        currentBytes = j2
        this.data = data
        child = downloadEpisodeCached
    }

    fun getData(): DownloadHeaderCached {
        return data
    }

    fun getChild(): DownloadEpisodeCached? {
        return child
    }
}